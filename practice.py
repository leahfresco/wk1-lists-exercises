...
def halvesies(numbers):
    ...
    half_numbers = []
    for number in numbers:
        half_numbers.append(float(number)/ 2)
    return half_numbers
...
def mult_numbers(numbers):
    ...
    mult_all_numbers = 1
    for number in numbers:
        mult_all_numbers *= number
    return mult_all_numbers
...
def average(numbers):
    ...
    for number in numbers:
        sum_all_numbers += number
    return float(sum_all_numbers)/ len(numbers)
...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    for index in range(len_items):
        reversed_list.append(items[len_items - index - 1])
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    beg = 0
    while beg < end:
        items[beg], items[end] = items[end], items[beg]
        end -= 1
        beg += 1
    ...
    for index in range(len(sorted_items)):
        if index != len(sorted_items) - 1:
            if sorted_items[index] == sorted_items[index + 1]:
                if sorted_items[index] not in duplicate_list:
                    duplicate_list.append(sorted_items[index])
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:
        for index_letter in range(len(word)):
            if word[index_letter] == letter:
                letter_indices.append(index_letter)
                break
            elif index_letter == len(word) - 1:
